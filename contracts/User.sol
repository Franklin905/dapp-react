...
contract User{
	...
	mapping(address=>bool) internal user_exist;
	...
	function view_user_exist_or_not(address _address) external view returns(bool){
		return user_exist[_address];
	}
	/*function view_user_intro(address _address)  external view returns(string memory){
		return users[address_to_account_index[_address]].intro;
	}
	function view_user_created_project_length(address _address)  external view returns(uint){
		return users[address_to_account_index[_address]].created_project.length;
	}
	function view_user_created_project(address _address,uint i)  external view returns(string memory){
		return users[address_to_account_index[_address]].created_project[i];
	}
	function view_user_sended_project_length(address _address)  external view returns(uint){
		return users[address_to_account_index[_address]].sended_project.length;
	}
	function view_user_sended_project(address _address,uint i)  external view returns(string memory){
		return users[address_to_account_index[_address]].sended_project[i];
	}
	function view_user_sended_money_length(address _address)  external view returns(uint){
		return users[address_to_account_index[_address]].sended_money.length;
	}
	function view_user_sended_money(address _address,uint i)  external view returns(uint){
		return users[address_to_account_index[_address]].sended_money[i];
	}*/
	...
	/*function view_user_started_voting_or_not(string calldata _title) external view returns(uint){
		return project_voting[msg.sender][_title];
	}*/
	function user_change_intro(string calldata _intro) external{
        users[address_to_account_index[msg.sender]].intro = _intro;
    }

	function view_user_voting_state(address _address, string calldata _title) external view returns(uint){
		return project_voting[_address][_title];
	}
	...
	function user_send_money_to_project(address _address, string memory _title, uint _value) internal{
		...
		for(i = 0 ; i<users[address_to_account_index[_address]].sended_project.length; i++){
			if(compareStrings(_title,users[address_to_account_index[_address]].sended_project[i])){
